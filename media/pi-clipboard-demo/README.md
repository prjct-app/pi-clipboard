# pi-clipboard promotional demo

This 40-second product film introduces pi-clipboard for PI Agent developers. It follows the package's monochrome cover identity and demonstrates the real workflow: paste an image, preview one or several attachments, and send only when ready.

[Watch or download the MP4](pi-clipboard-demo.mp4)

## Media details

- 1920 × 1080 at 30 fps
- H.264 video with AAC stereo audio
- English on-screen copy
- No voice-over
- Original instrumental soundtrack with no external samples
- Monochrome palette matching the package cover

The MP4 is stored outside the npm package contents so installing the extension does not download promotional media.
